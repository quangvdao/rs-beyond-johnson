...
\maketitle
\begin{abstract}
This companion studies three questions about hidden-derivative interpolation
for Reed--Solomon codes: how many independent constraints its local map
imposes, how to enumerate differential roots when the characteristic limits
Taylor reconstruction, and how large agreement lists can become as the gap
from capacity shrinks. We give exact rank formulas and finite certificates,
root-enumeration bounds using field descent and resonant coefficients, and
list-size constructions for arbitrary evaluation sets. A separate obstruction
shows why equations supported only on higher derivatives can have too many
roots in small characteristic. The main paper develops the decoding and
correlated-agreement bounds; here we examine the alternative supports and
limitations that complement those results.
\end{abstract}
\section{Introduction}
\label{sec:companion-introduction}

An interpolation polynomial can certify that every close message solves a
differential equation without providing a useful way to enumerate all its
solutions. Nor does an asymptotic interpolation bound tell us the exact rank
of a finite constraint system. These distinctions motivate this companion
to \emph{Reed--Solomon Codes Beyond Johnson: Efficient Decoding and Smaller
Cryptographic Proofs}.

\paragraph{Finite interpolation.}
\Cref{sec:exact-certificate} computes the local kernel and separates the
exact rank of an enlarged map from that of the actual interpolation map.
The resulting formulas support explicit small-order certificates and finite
comparisons in \Cref{sec:optimized-parameters,sec:quantitative-refinements}.
For an alternative one-sided support, \Cref{app:boundary-asymptotics}
derives a fixed-gap boundary law directly from the exact dimension formula.

\paragraph{Differential roots.}
\Cref{sec:root-finding,sec:refinements} explain how regular Taylor data
determine a solution, how vanishing recurrence coefficients introduce free
choices, and how descent to the field generated by the input reduces
enumeration cost. These bounds enumerate differential roots before checking
agreement. The main paper's faster decoders instead use agreement to filter
the family before enumeration.

\paragraph{Obstructions.}
\Cref{sec:characteristic} constructs large differential-root families in
small characteristic; the obstruction concerns the specified support,
not every possible decoder. \Cref{sec:sharpness} constructs large agreement
lists and distinguishes a fixed positive gap from a gap that shrinks with
block length. These constructions do not give matching lower bounds on
interpolation order.

\paragraph{Conventions and supporting material.}
We recall the definitions needed here. References marked ``main paper''
link to \href{rs-beyond-johnson.pdf}{the accompanying PDF} and use its
numbering. Application certificates and measurement procedures are documented
for ProveKit, ZisK, and LambdaVM in the repository's
\href{https://github.com/quangvdao/rs-beyond-johnson/blob/main/CALCULATIONS.md}{calculation guide}. The main paper's AI-assistance disclosure applies
to both manuscripts; the authors take responsibility for the statements and
proofs here. Its formalization claims do not extend to every result in this
companion.
...
We use $(x)_+=\max\{x,0\}$, the indicator $\mathbf1_{\{\mathcal P\}}$,
and natural logarithms. For vectors of nonnegative exponents, $|x|$ denotes
the sum of the coordinates. The boundary asymptotic uses Lebesgue volume,
expectation, and the harmonic sums
$H_t=\sum_{j=1}^t j^{-1}$ and $H_t^{(2)}=\sum_{j=1}^t j^{-2}$.
Subscripts on $O(\cdot)$ indicate the parameters on which its constant depends.
...
The backward identity recovers a value from derivatives at the shifted
point. It is the input to the hidden-derivative multiplicity argument.
...
At a center $a$, an order-$r$ \emph{jet} is the tuple
$(P(a),P^{[1]}(a),\ldots,P^{[r]}(a))$ of coefficients in $P(a+T)$.
A polynomial differential equation $Q(X,Y_0,\ldots,Y_r)=0$ is solved by
$P$ when $Q(X,P,P^{[1]},\ldots,P^{[r]})$ vanishes identically.
Its \emph{order} is the highest derivative index present; its \emph{jet degree}
is total degree in the $Y_j$, ignoring $X$.

For an equation of order $r$, the \emph{separant} is $S=\partial Q/\partial Y_r$.
A jet at $a$ is regular if $S(a,P(a),\ldots,P^{[r]}(a))\ne0$.
The separant multiplies each new unknown in the Taylor recurrence; when the
other scalar multipliers are also nonzero, the recurrence determines later
coefficients. The root-counting sections state the characteristic assumptions
that make this reconstruction valid.
...
\]
Distinct source monomials that are nonzero modulo \(T^m\) have distinct target
...
The certificates below use explicit parameters independent of block length.
At order zero a linear equation in the message gives uniqueness; at order
one an exact local-rank calculation certifies agreement $k+\lceil n/4\rceil$
uniformly over the rate. These cases also supply the large-gap branches of
\Cref{cor:linear-characteristic-decoding}.
...
$o(n)$, so the main paper's finite length condition
\[
...
We evaluate first-order and Johnson bounds at finite block lengths, and
compare the resulting constants with full differentiation. The tables use
exact integer parameter choices to expose losses hidden by asymptotic notation.
...
\end{table}
 \section{Differential roots from Taylor data}
\label{sec:root-finding}

We count and enumerate all polynomial solutions of a differential equation,
using Kopparty's reconstruction from regular initial Taylor
jets~\cite[Theorem~4.3, Theorem~4.4, and Corollary~4.5]{Kopparty15}.
A regular jet determines at most one solution below the characteristic.
Passing to derivative equations supplies regular witnesses for the remaining
solutions. Counting these witnesses over a finite extension bounds the full
root set; agreement filtering may leave a smaller list.
...
differential roots; checking received-word agreement then selects the
requested messages.
...
\begin{proof}
The individual jet degrees are at most $\Delta<q$, and
\eqref{eq:chain-singularity-generic-bound} gives $H_{\rm ch}(Q)\le h$.
Apply \Cref{thm:root-bound} and use $S-H_{\rm ch}(Q)\ge S-h>0$.
\end{proof}

For the resonance analysis below, we also need to group consecutive
steps of the derivative chain by their active order. Let $R$ be a nonzero
iterated partial derivative of $Q$ over $E=\F_S$. If $R$ depends on jet
variables, let $s$ be its largest active order and $t=\deg_{Y_s}R$. Put
\begin{equation}
 R_a=\frac{\partial^aR}{\partial Y_s^a},\qquad 0\le a\le t.
 \label{eq:partial-derivative-chain}
\end{equation}
Since $t<q$, the terminal polynomial
\begin{equation}
 R_t=t!\operatorname{LC}_{Y_s}(R)
 \label{eq:terminal-partial-derivative}
\end{equation}
is nonzero and has smaller active order. Write $\mathcal Z_D(R)$ for
the differential root set of $R$ in $E[X]_{\le D}$. The roots of $R$ split
according to their first nonzero derivative specialization: for some
$0\le a<t$, they satisfy
\begin{equation}
 R_b(X,P,\ldots,P^{[s]})\equiv0\quad(0\le b\le a),\qquad
 R_{a+1}(X,P,\ldots,P^{[s]})\not\equiv0.
 \label{eq:regular-stratum}
\end{equation}
These are the regular multiplicity strata. The remaining roots belong to
$\mathcal Z_D(R_t)$, so we continue with the lower-order equation.
At a point where $R_{a+1}$ is nonzero, the last jet coordinate is a root
of exact multiplicity $a+1$ of
$R(\alpha,\beta_0,\ldots,\beta_{s-1},Z)$. Thus all regular strata together
allow at most $t$ last coordinates for each fixed lower jet. Moreover,
$\deg_YR_t\le\deg_YR-t$, so the degrees $t$ encountered along the
residual recursion sum to at most $\Delta$.

The enumeration guarantee in \Cref{thm:root-bound} likewise has the
following parameter-only form.
...
\begin{proof}
Apply the enumeration algorithm of \Cref{thm:root-bound}, since
$S>h\ge H_{\rm ch}(Q)$. A representation of $E=\F_{q^e}$ can be
constructed within the same bound by enumerating monic degree-$e$
polynomials over $\F_q$ and testing irreducibility.
\end{proof}
...
degree. The root bounds above apply to either polynomial.
...
Using the decomposition in \eqref{eq:regular-stratum}, the roots of \(R\)
...
For a fiber of degree \(t\), at most \(t\) top jet values are retained.  The
...
Kominers thanks MIT Sloan's Technological Innovation, Entrepreneurship,
and Strategic Management (TIES) Group for its hospitality during this work.
 \section*{Disclosure}
...
\(K=\lfloor(1-\theta)\varepsilon n\rfloor\).  For agreement $A=k+\lceil\delta n\rceil$, $0<\delta<1$, one may instead
choose $K=\max\{k,\lceil\rho_0n\rceil\}$ with
$\rho_0=\delta(1-\delta)/2$. Since $k\ge1$ and
$\lceil\rho_0n\rceil\le\lceil\delta n\rceil$, both entries of the maximum
are strictly smaller than $A$. Moreover,
$(K-1)/n\ge\rho_0-1/n\ge\rho_0/2$ for $n\ge2/\rho_0$.
The main paper uses a different ambient baseline in
...
constant. The limit keeps the rate and agreement fixed while $d\to\infty$;
it does not justify a joint limit as the gap tends to zero.
